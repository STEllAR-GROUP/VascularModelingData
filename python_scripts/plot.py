...
def plot_general_with_labels(filename):
    ...
    j=0
    for (middle, level) in zip(middles,levels):
        plt.axvline(level,linestyle='dashed',color='gray',linewidth=2)
        if level<0:
            j=j+1
            plt.text(middle-0.01, ymax+0.01, str(j))            
        else:
    ...
    levels.append(xmax)
    plt.axvline(xmax,linestyle='dashed',color='gray',linewidth=2)       
    ...
    for ves in vessel_info.keys():
        ...
        if nodes[vessel_info[ves]['node_ids'][0]][0]>=0:
            ...
            for vid,end in zip(vessel_info[ves]['connected_vessels_labels'][0:len(c_nodes)],c_nodes):
                ...
                plt.figure(1)
                plt.text(middle_x,middle_y+0.002,vid,color='gray')
...
plt.plot([nodes['0'][0],nodes['1'][0]],[nodes['0'][1],nodes['1'][1]],marker='o')   
plt.text(nodes['0'][0]-0.003,nodes['0'][1]+0.01,'0') 
plt.text(nodes['1'][0]+0.0001,nodes['1'][1]+0.01,'1') 
